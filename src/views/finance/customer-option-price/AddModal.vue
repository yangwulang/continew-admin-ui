...
import { addFinCustomerOptionPrice, getFinCustomerOptionPrice, updateFinCustomerOptionPrice } from '@/apis/finance/fin-customer-option-price'
...
const title = computed(() => (isUpdate.value ? '修改客户选项价格' : '新增客户选项价格'))
const formRef = ref<InstanceType<typeof GiForm>>()

const customerDict = ref<LabelValueState[]>([])
const loadCustomerDict = async () => { const { data } = await listFinCustomerDict(); customerDict.value = data || [] }
...
const [form, resetForm] = useResetReactive({ customerId: undefined, optionId: undefined, priceMode: 'PER_PAGE', price: 0, effectiveFrom: undefined, effectiveTo: undefined })

const columns: ColumnItem[] = reactive([
  { label: '客户', field: 'customerId', type: 'select', span: 24, required: true, props: { options: customerDict, allowSearch: true, allowClear: true, placeholder: '请选择客户' } },
...
const reset = () => { formRef.value?.formRef?.resetFields(); resetForm() }

const save = async () => {
  try {
    const isInvalid = await formRef.value?.formRef?.validate()
    if (isInvalid) return false
    if (isUpdate.value) { await updateFinCustomerOptionPrice(form, dataId.value); Message.success('修改成功') }
    else { await addFinCustomerOptionPrice(form); Message.success('新增成功') }
    emit('save-success'); return true
  } catch { return false }
}

const onAdd = async () => { reset(); dataId.value = ''; await Promise.all([loadCustomerDict(), loadOptionDict()]); visible.value = true }
const onUpdate = async (id: string) => { reset(); dataId.value = id; await Promise.all([loadCustomerDict(), loadOptionDict()]); const { data } = await getFinCustomerOptionPrice(id); Object.assign(form, data); visible.value = true }
